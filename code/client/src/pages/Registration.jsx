...
const Registration = () => {
  ...
  useEffect(() => {
    async function fetchData() {
        let res = await getCourses({ account_id: loginInfo.account_id });
        setCourses(res.data);
    }
    fetchData();
  ...
  const [registerCurrCourse, setRegisterCurrCourse] = useState();

  function handleSubmit(){
    let res = registerCourse({ account_id: loginInfo.account_id, course_id: selectCourse });
    setRegisterCurrCourse(res.data);
  }
        ...
        <div className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg m-3 p-4 rounded-2xl md:w-1080">
          ...
          <div className="mt-4">
            <form class="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
              ...
              <button type="submit" style={{ backgroundColor:currentColor }} className="w-full text-white bg-blue-600 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Register Course</button>
            </form>
          </div>
